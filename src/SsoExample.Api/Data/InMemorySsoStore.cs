...
public sealed class InMemorySsoStore
{
    ...
    [
        new("ssoexample-web", "SSOExample.Web SPA")
    ];

    private readonly List<RefreshTokenRecord> _refreshTokens = [];
    private readonly List<AuditLogRecord> _auditLogs = [];
    private readonly List<RequestLogRecord> _requestLogs = [];
    private readonly object _requestLogLock = new();
    ...
    public IReadOnlyList<RequestLogRecord> RequestLogs
    {
        get
        {
            lock (_requestLogLock)
            {
                return _requestLogs.OrderByDescending(x => x.CreatedAt).Take(500).ToList();
            }
        }
    }

    public void AddRequestLog(RequestLogRecord record)
    {
        lock (_requestLogLock)
        {
            _requestLogs.Add(record);
            if (_requestLogs.Count > 1000)
            {
                _requestLogs.RemoveRange(0, _requestLogs.Count - 1000);
            }
        }
    }
