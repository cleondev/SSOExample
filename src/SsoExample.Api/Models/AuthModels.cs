...
public sealed record RequestLogRecord(
    ...
    string UserAgent,
    bool Impersonated,
    Guid? ImpersonationActorUserId,
    string? ImpersonationActorUserName);
...
public sealed record CurrentPrincipal(Guid UserId, string UserName, string Email, string DisplayName, string[] Roles, ImpersonationInfo? Impersonation);
